...
## Development Day 7: 2023.11.25

1. `Nav Mesh Agent`组件：导航系统——导航系统需要自己的数据来表示游戏场景中的可行走区域。可行走区域定义了代理可在场景中站立和移动的位置。在 Unity 中，代理被描述为圆柱体。可行走区域是通过测试代理可站立的位置从场景中的几何体自动构建的。然后，这些位置连接到场景几何体之上覆盖的表面。该表面称为导航网格（简称 NavMesh）。

2. Unity当中移动物体有很多种方法，常用的有三种：
   - `CharacterConrtoller`
   - 移动`RigidBody` 刚体
   - `Vector3` 的`MoveTowards`函数（不是很好用，移动不够精细）

3. `NavMeshAgent.destination`：说白了就是有`agent`的物体移动到的目标
   1. 获取代理在世界坐标系单位中的目标或尝试设置代理在其中的目标（摘自Unity）。

4. 可以再`Adobe`的动画网站`mixamo`上下载动画，对于不了解3D动画的新手来说，大概有这么几个坑（挨个踩过）：

   1. 首先选择`Character`下载能下载一个角色的模型贴图以及相关的动画，选择`Animation`则只能下载一个类似模具一样的角色。模具一样的应该是只下载了动画的骨骼，而没有下载动画的皮肤，可以在这个骨骼上贴上任意的皮肤。

   2. 其次，将`mixamo`下载的动画导入`Unity`之后会出现一些问题，比如只有白模而没有贴图材质，这个时候要在动画选项的`Materials`部分，将参数更改如下：

      ![image-20231125174230427](Images/解决模型白模问题.png)

      之后点击`Apply`，可能会有个弹窗跳出来告诉你有些东西缺了，这个时候直接点击`Fix`，`Unity`会帮助你生成该有的东西的。记住更改之后要点击`Apply`才能正式开始应用。

   3. 最后，在导入之后会发现整个模型没有动画的很多东西，例如`Avatar`文件，这个时候要也要调整一些参数，将`Rig`当中的参数更改如下：

      ![image-20231125174759245](Images/解决Avatar问题_2.png)

      选择`Humanoid`意思是告诉`Unity`这个模型是个拟人模型。`Animation Type`参数的意思如下：
      ...
      **注意：`Avatar Definition`的参数并不是一定的，打个比方，上面这个是`Mutant`的一个动画，他可以顺利生成骨骼，但是`Mutant`的`T-pose`模型却不是，它在生成的时候总是会告诉我少了一只左手：**

      ![image-20231125175022749](Images/解决Avatar问题_3.png)
